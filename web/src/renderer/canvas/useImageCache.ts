...
export async function loadBitmap(url?: string): Promise<ImageBitmap | HTMLImageElement | null> {
  if (!url) return null;
  ...
  try {
    ...
    if (normalizedUrl.startsWith('blob:') || normalizedUrl.startsWith('http') || normalizedUrl.startsWith('data:') || normalizedUrl.startsWith('file:')) {
